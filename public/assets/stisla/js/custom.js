...
function Notification(){
	...
	var	cardH = '<div class="card-header">';
	var cardB = '<div class="card-body">'
	var close = '</div>';
	var	closea = '</a>';
	$.ajax({
		type:'post',
		url:'https://famerentcar.innco.tech/Notification/list',
		dataType: 'json',
		success: function(response){
		//console.log(response['length']);
		if(response['length']==0){
			$('#isi-notif').html('');
			$('#isi-notif').append('<a id="notif-kosong" class="dropdown-item">Belum ada notifikasi</a>');
			$('#isi-badan').append(row + col + '<div id="notif-card" class="card">' + cardH +'Belum ada notifikasi' +  close + close + close + close);
		}

		$.each(response,function(index,value){
			if(response[index]["is_read"] == 0){
				$('#notif-btn').addClass('beep');
				$('#isi-badan').append(row + col + '<div id="notif-card" class="card beep">' + cardH + response[index]['notification'] + close + close + close + close);
				$('#isi-notif').append('<a id="notif-'+response[index]['notif_id']+'" href="https://famerentcar.innco.tech/notification/'+response[index]['notif_id']+'" class="dropdown-item dropdown-item-unread beep"><div class="dropdown-item-icon bg-primary text-white"><i class="fas fa-bell"></i></div><div class="dropdown-item-desc">' + response[index]["notification"] + '<div id="notif-'+response[index]['notif_id']+'-time" class="time text-primary">2 Min Ago</div></div></a>');
			}else{
				$('#isi-notif').append('<a id="notif-'+ response[index]['notif_id']+'" href="https://famerentcar.innco.tech/notification/'+response[index]['notif_id']+'" class="dropdown-item dropdown-item-unread "><div class="dropdown-item-icon bg-primary text-white"><i class="fas fa-bell"></i></div><div class="dropdown-item-desc">' + response[index]["notification"] + '<div id="notif-'+response[index]['notif_id']+'-time" class="time text-primary">2 Min Ago</div></div></a>');
				$('#isi-badan').append(row + col + '<div id="notif-card" class="card">' + cardH + response[index]['notification'] + close + close + close + close);
			}
			//console.log(response[index]['time'])
						var time = new Date(response[index]['time']);
						var now = new Date();
						var x = (now - time);
						...
						hr = hr %24;
						if(min==0 && hr==0 && day ==0 && sec>0){
						$('#notif-' + response[index]['id'] + '-time').text('Beberapa detik yang lalu');
						}
						if(day>0){
						$('#notif-' + response[index]['id'] + '-time').text(day + ' Hari ' + jam + ' Jam' + ' yang lalu');	
						}
						if(hr>0){
						$('#notif-' + response[index]['id'] + '-time').text(hr + ' Jam ' + min + ' Menit' + ' yang lalu');	
						}
						if(min>0 && hr ==0 && day==0){
						$('#notif-' + response[index]['id'] + '-time').text(min + ' Menit ' + sec + ' Detik' + ' yang lalu')
						}
		})
...
$(document).ready(function(){
	...
	$.ajax({
    type:'post',
    url:'https://famerentcar.innco.tech/online',
    dataType: 'json',
    success: function(response){
...
//	$('#myModal').modal('toggle')
	...
	//console.log('dokumen ready');
	if(!$.trim($('div#isiorderlist').html())){
		$('div#isiorderlist').append('<div class="col-12"><div class="card"><div class="card-header">Belum ada order masuk</div></div></div>');
	}
	if(!$.trim($('div#isiperjalanan').html())){
		$('div#isiperjalanan').append('<div class="col-12"><div class="card"><div class="card-header">Belum ada perjalanan</div></div></div>');
	}
	
	var interval = setInterval(function() {
		
		var durasi = $("#start-time").text();
		var f = new Date(durasi);
		var now = new Date();
		var x = (now - f);
		var dh = 0, hh=0, tot = 0, price = Number($('#harga').text());
		var sec = Math.floor(x/1000);
		var min = Math.floor(sec/60); 
		sec = sec % 60;
		var hr = Math.floor(min/60);
		...
		if(hr==0 && day == 0 && min <30){
			$('#payment').text('Rp ' + $('#harga').text());
		}else if(hr>0 && day==0 && min<30){
			tot = (hr*price);
			$('#payment').text('Rp ' + tot);
		}else if(hr>0 && day==0 && min>=30){
			tot = (hr*price) + 15000;
			...
			tot = (24*price) + (hr*price) + 15000;
			$('#payment').text('Rp ' + tot);
		}else if(hr>0 && day>0 && min<30){
			tot = (24*price) + (hr*price);
			$('#payment').text('Rp ' + tot);
		}
	...
	var notifcheck = setInterval(function() {
		var row = '<div class="row">';
	var col = '<div class="col-lg-12">';
	var card = '<div class="card">';
	var	cardH = '<div class="card-header">';
	...
	var close = '</div>';
	var	closea = '</a>';
		var notifring = document.getElementById('#Ringing');
		$.ajax({
		type:'post',
		url:'https://famerentcar.innco.tech/Notification/list',
		dataType: 'json',
			success: function(response){
				//console.log(response);
				$.each(response,function(index,value){
					if(response[index]['is_read'] == 0 && response[index]['is_notify'] == 0){

						// $('#isi-badan').empty();
						// $('#isi-notif').empty();
						//$('#notif-side').remove('href');
						//console.log(response[index]['id']);
						if($('#notif-kosong').text() == "Belum ada notifikasi"){
							//console.log('ada loh mek');
							$('#isi-notif').html('');
							$('#isi-badan').html('');
						}

						if($('#notif-'+ response[index]['id']).attr('href')){
						//console.log('sudeh');
						}else{
						$('#isi-badan').prepend(row + col + '<div id="notif-card" class="card beep">' + cardH + response[index]['notification'] + close + close + close + close);
						$('#isi-notif').prepend('<a id="notif-'+response[index]['id']+'" href="https://famerentcar.innco.tech/notification/'+response[index]['id']+'" class="dropdown-item dropdown-item-unread beep"><div class="dropdown-item-icon bg-primary text-white"><i class="fas fa-bell"></i></div><div class="dropdown-item-desc">' + response[index]["notification"] + '<div id="notif-'+response[index]['id']+'-time" class="time text-primary"></div></div></a>');
						}
						var time = new Date(response[index]['time']);
						var now = new Date();
						var x = (now - time);
...
Swal.fire({
  title: 'Informasi',
  text: "Anda yakin menyelesaikan rental ini ?",
  icon: 'info',
  ...
  if (result.value) {
    ...
    Swal.fire(
      'Order Selesai',
      'Selamat order anda selesai saatnya pembayaran !',
      'success'
    )
  }
})
...
//     content: '<form><input placeholder="waw"></form><button class="btn btn-primary btn-rounded" data-izimodal-close="modal-custom" data-izimodal-transitionout="bounceOutDown">X</button>',
//     default: false
// 	})
//   	e.preventDefault();
...
// $(".modal-login-close").on('click', function(e){
